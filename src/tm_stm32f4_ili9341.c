...
/* Ports definitions */
#define CS_MUX_GROUP     6		//CS = GPIO0: Pin29
#define CS_MUX_PIN       1
...
// Se supone que ya se hizo un boardConfig();
...
void TM_ILI9341_VerticalScrollingDefinition (uint16_t tfa, uint16_t vsa, uint16_t bfa) {
	TM_ILI9341_SendCommand(ILI9341_VSCRDEF);
	TM_ILI9341_SendData(tfa >> 8);
	TM_ILI9341_SendData(tfa & 0xFF);
	TM_ILI9341_SendData(vsa >> 8);
	TM_ILI9341_SendData(vsa & 0xFF);
	TM_ILI9341_SendData(bfa >> 8);
	TM_ILI9341_SendData(bfa & 0xFF);
}

void TM_ILI9341_VerticalScrollingStartAddress  (uint16_t vsp) {
	TM_ILI9341_SendCommand(ILI9341_VSCRSADD);
	TM_ILI9341_SendData(vsp >> 8);
	TM_ILI9341_SendData(vsp & 0xFF);
}
